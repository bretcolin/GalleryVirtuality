...
// note, the 'maxVerts' param is purely a hint as to how big to make the STL optimise arrays so they don't need to
...
void OrderMesh( int input[][3], int output[][3], int numTris, int maxVerts )
{
	...
	TriangleAbutList_Gen(input, numTris, maxVerts);
	...
	{
		printf("%c%d%%",0x0D,(sumStrippedTriangles*100)/numTris);
		// build a strip
