...
;784:	RegisterItem( BG_FindItemForWeapon( WP_HAND ) );
CNSTI4 1
ARGI4
ADDRLP4 0
...
pop
line 785
;785:	RegisterItem( BG_FindItemForWeapon( WP_HAND ) );
CNSTI4 1
ARGI4
ADDRLP4 4
ADDRGP4 BG_FindItemForWeapon
...
;787:	if( g_gametype.integer == GT_HARVESTER ) {
;788:		RegisterItem( BG_FindItem( "Red Cube" ) );
;789:		RegisterItem( BG_FindItem( "Blue Cube" ) );
;790:	}
;791:#endif
;792:}
LABELV $237
...
LABELV $239
line 805
;805:	itemRegistered[ item - bg_itemlist ] = qtrue;
ADDRFP4 0
INDIRP4
CVPU4 4
ADDRGP4 bg_itemlist
...
;809:/*
;810:===============
;811:SaveRegisteredItems
;812:
;813:Write the needed items to a config string
;814:so the client will know which ones to precache
;815:===============
;816:*/
;817:void SaveRegisteredItems( void ) {
line 822
;818:	char	string[MAX_ITEMS+1];
;819:	int		i;
;820:	int		count;
;821:
;822:	count = 0;
...
line 833
;832:
;833:	G_Printf( "%i items registered\n", count );
ADDRGP4 $249
ARGP4
ADDRLP4 264
INDIRI4
...
pop
line 834
;834:	trap_SetConfigstring(CS_ITEMS, string);
CNSTI4 27
ARGI4
ADDRLP4 4
ARGP4
...
;840:============
;841:*/
;842:int G_ItemDisabled( gitem_t *item ) {
line 846
;843:
;844:	char name[128];
;845:
;846:	Com_sprintf(name, sizeof(name), "disable_%s", item->classname);
ADDRLP4 0
ARGP4
CNSTI4 128
ARGI4
...
pop
line 847
;847:	return trap_Cvar_VariableIntegerValue( name );
ADDRLP4 0
ARGP4
ADDRLP4 128
...
;852:G_SpawnItem
;853:
;854:Sets the clipping size and plants the object on the floor.
;855:
;856:Items can't be immediately dropped to floor, because they might
;857:be on an entity that hasn't spawned yet.
;858:============
;859:*/
;860:void G_SpawnItem (gentity_t *ent, gitem_t *item) {
line 861
;861:	G_SpawnFloat( "random", "0", &ent->random );
ADDRGP4 $253
ARGP4
ADDRGP4 $254
ARGP4
...
pop
line 862
;862:	G_SpawnFloat( "wait", "0", &ent->wait );
ADDRGP4 $255
ARGP4
ADDRGP4 $254
ARGP4
...
ASGNP4
line 871
;869:	// some movers spawn on the second frame, so delay item
;870:	// spawns until the third frame so they can ride trains
;871:	ent->nextthink = level.time + FRAMETIME * 2;
ADDRFP4 0
INDIRP4
CNSTI4 688
ADDP4
...
ASGNI4
line 872
;872:	ent->think = FinishSpawningItem;
ADDRFP4 0
INDIRP4
CNSTI4 692
ADDP4
...
line 874
;873:
;874:	ent->physicsBounce = 0.50;		// items are bouncy
ADDRFP4 0
INDIRP4
CNSTI4 568
ADDP4
...
NEI4 $259
line 877
;877:		G_SoundIndex( "sound/items/poweruprespawn.wav" );
ADDRGP4 $142
ARGP4
ADDRGP4 G_SoundIndex
CALLI4
pop
line 878
;878:		G_SpawnFloat( "noglobalsound", "0", &ent->speed);
ADDRGP4 $261
ARGP4
ADDRGP4 $254
ARGP4
...
;882:	if ( item->giType == IT_PERSISTANT_POWERUP ) {
;883:		ent->s.generic1 = ent->spawnflags;
;884:	}
;885:#endif
;886:}
LABELV $252
...
;893:================
;894:*/
;895:void G_BounceItem( gentity_t *ent, trace_t *trace ) {
line 901
;896:	vec3_t	velocity;
;897:	float	dot;
;898:	int		hitTime;
;899:
;900:	// reflect the velocity on the trace plane
;901:	hitTime = level.previousTime + ( level.time - level.previousTime ) * trace->fraction;
ADDRLP4 16
ADDRGP4 level+36
INDIRI4
CVIF4 4
...
ASGNI4
line 902
;902:	BG_EvaluateTrajectoryDelta( &ent->s.pos, hitTime, velocity );
ADDRFP4 0
INDIRP4
CNSTI4 12
ADDP4
...
pop
line 903
;903:	dot = DotProduct( velocity, trace->plane.normal );
ADDRLP4 20
ADDRFP4 4
INDIRP4
ASGNP4
...
ASGNF4
line 904
;904:	VectorMA( velocity, -2*dot, trace->plane.normal, ent->s.pos.trDelta );
ADDRFP4 0
INDIRP4
CNSTI4 36
ADDP4
...
line 907
;905:
;906:	// cut the velocity to keep from bouncing forever
;907:	VectorScale( ent->s.pos.trDelta, ent->physicsBounce, ent->s.pos.trDelta );
ADDRLP4 24
ADDRFP4 0
INDIRP4
ASGNP4
...
;908:
;909:	// check for stop
;910:	if ( trace->plane.normal[2] > 0 && ent->s.pos.trDelta[2] < 40 ) {
ADDRFP4 4
INDIRP4
CNSTI4 32
ADDP4
...
GEF4 $270
line 911
;911:		trace->endpos[2] += 1.0;	// make sure it is off ground
ADDRLP4 48
ADDRFP4 4
INDIRP4
CNSTI4 20
...
ASGNF4
line 913
;913:		G_SetOrigin( ent, trace->endpos );
ADDRFP4 0
INDIRP4
ARGP4
ADDRFP4 4
...
pop
line 914
;914:		ent->s.groundEntityNum = trace->entityNum;
ADDRFP4 0
INDIRP4
CNSTI4 148
ADDP4
...
;916:	}
;917:
;918:	VectorAdd( ent->r.currentOrigin, trace->plane.normal, ent->r.currentOrigin);
ADDRLP4 48
ADDRFP4 0
INDIRP4
CNSTI4 488
...
ASGNF4
line 919
;919:	VectorCopy( ent->r.currentOrigin, ent->s.pos.trBase );
ADDRLP4 60
ADDRFP4 0
INDIRP4
ASGNP4
...
ASGNB 12
line 920
;920:	ent->s.pos.trTime = level.time;
ADDRFP4 0
INDIRP4
CNSTI4 16
ADDP4
...
;928:================
;929:*/
;930:void G_RunItem( gentity_t *ent ) {
line 937
;931:	vec3_t		origin;
;932:	trace_t		tr;
;933:	int			contents;
;934:	int			mask;
;935:
;936:	// if groundentity has been set to -1, it may have been pushed off an edge
;937:	if ( ent->s.groundEntityNum == -1 ) {
ADDRFP4 0
INDIRP4
CNSTI4 148
ADDP4
...
NEI4 $274
line 938
;938:		if ( ent->s.pos.trType != TR_GRAVITY ) {
ADDRFP4 0
INDIRP4
CNSTI4 12
ADDP4
...
EQI4 $276
line 939
;939:			ent->s.pos.trType = TR_GRAVITY;
ADDRFP4 0
INDIRP4
CNSTI4 12
ADDP4
...
line 944
;943:
;944:	if ( ent->s.pos.trType == TR_STATIONARY ) {
ADDRFP4 0
INDIRP4
CNSTI4 12
ADDP4
...
;951:	BG_EvaluateTrajectory( &ent->s.pos, level.time, origin );
ADDRFP4 0
INDIRP4
CNSTI4 12
ADDP4
...
line 954
;952:
;953:	// trace a line from the previous position to the current position
;954:	if ( ent->clipmask ) {
ADDRFP4 0
INDIRP4
CNSTI4 572
...
LABELV $282
line 957
;957:		mask = MASK_PLAYERSOLID & ~CONTENTS_BODY;//MASK_SOLID;
ADDRLP4 72
CNSTI4 65537
ASGNI4
line 958
...
LABELV $283
line 959
;959:	trap_Trace( &tr, ent->r.currentOrigin, ent->r.mins, ent->r.maxs, origin, 
ADDRLP4 0
ARGP4
ADDRLP4 76
ADDRFP4 0
...
pop
line 962
;960:		ent->r.ownerNum, mask );
;961:
;962:	VectorCopy( tr.endpos, ent->r.currentOrigin );
ADDRFP4 0
INDIRP4
CNSTI4 488
ADDP4
...
line 968
;967:
;968:	trap_LinkEntity( ent );	// FIXME: avoid this for stationary?
ADDRFP4 0
INDIRP4
ARGP4
ADDRGP4 trap_LinkEntity
...
;975:	}
;976:
;977:	// if it is in a nodrop volume, remove it
;978:	contents = trap_PointContents( ent->r.currentOrigin, -1 );
ADDRFP4 0
INDIRP4
CNSTI4 488
ADDP4
...
ASGNI4
line 979
;979:	if ( contents & CONTENTS_NODROP ) {
ADDRLP4 68
INDIRI4
CVIU4 4
CNSTU4 2147483648
...
EQU4 $292
line 980
;980:		if (ent->item && ent->item->giType == IT_TEAM) {
ADDRLP4 84
ADDRFP4 0
INDIRP4
CNSTI4 812
