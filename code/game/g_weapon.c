...
Info Sight Stuff
	...
	Info Sight / Flash Light Functions
============
*/

void Info_Gen( gentity_t *ent, int type )	{
	gentity_t	*las;
	int oldtype;

	//Get rid of you?
	if ( ent->client->infosight) {
		  oldtype = ent->client->infosight->s.eventParm;
		  G_FreeEntity( ent->client->infosight );
		  ent->client->infosight = NULL;
		  if (oldtype == type)
			  return;
	}
	...
	las->think = Info_Think;
	las->r.ownerNum = ent->s.number;
	las->parent = ent;
	las->s.eType = ET_INFO;

	//Lets tell it if flashlight or info
	if (type == 2)	{
		las->s.eventParm = 2; //tells CG that it is a flashlight
		las->classname = "flashlight";
	}
	else {
		las->s.eventParm = 1; //tells CG that it is a info sight
		las->classname = "infosight";
	}

	ent->client->infosight = las;
}

void Info_Think( gentity_t *self )	{
...
void Laser_Gen( gentity_t *ent, int type )	{
	...
	if (type == 1)	{
		las->s.eventParm = 1; //tells CG that it is a lasersight
